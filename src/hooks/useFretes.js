...
export const useFretes = (empresaId) => {
  ...
  const carregarFretes = useCallback(async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${API_URL}/api/fretes/empresa/${empresaId}`,
        {
  ...
  // Criar novo frete
  const criarFrete = useCallback(
    async (freteId, freteData) => {
      try {
        setLoading(true);
        setError(null);
        ...
        const novoFreteData = {
          ...
          valorEmpresa: parseFloat(
            (freteData.valorEmpresa || "0")
              .toString()
              .replace(/[R$\s,]/g, "")
              .replace(".", "")
          ),
        ...
        const response = await fetch(`${API_URL}/api/fretes`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const editarFrete = useCallback(
    async (freteId, freteData) => {
      try {
        ...
        const response = await fetch(`${API_URL}/api/fretes/${freteId}`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
  ...
  // Remover frete
  const removerFrete = useCallback(
    async (freteId) => {
      try {
        setLoading(true);
        setError(null);
  ...
  const oferecerFrete = useCallback(
    async (freteId, motoristaId) => {
      try {
        setLoading(true);
        setError(null);
        ...
        const response = await fetch(
          `${API_URL}/api/fretes/${freteId}/oferecer`,
          {
            method: "POST",
            headers: { "Content-Type": "application/json" },
  ...
  // Finalizar frete
  const finalizarFrete = useCallback(
    async (freteId) => {
      try {
        setLoading(true);
        setError(null);
        ...
        const response = await fetch(
          `${API_URL}/api/fretes/${freteId}/finalizar`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },

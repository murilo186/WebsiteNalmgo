...
export const useTeam = (empresaId) => {
  ...
  const carregarEquipe = useCallback(
    async (forceReload = false) => {
      ...
      try {
        ...
        const equipeResponse = await apiService.get(
          `/api/motoristas/empresa/${empresaId}/equipe`
        );
        ...
        try {
          const colaboradoresResponse = await fetch(
            `${API_URL}/api/auth/empresa/${empresaId}/colaboradores`
          );
          ...
          if (colaboradoresData && colaboradoresData.success) {
            ...
            console.log(
              ...
              colaboradoresData?.error
            );
            setColaboradores([]);
          }
        } catch (colaboradoresError) {
          console.log(
            "❌ Erro ao conectar com colaboradores:",
  ...
  const adicionarMotorista = useCallback(
    async (codigo) => {
      ...
      try {
        ...
        const response = await apiService.enviarConvite(
          empresaId,
  ...
  const adicionarColaborador = useCallback(
    async (dadosColaborador) => {
      ...
      try {
        ...
        const response = await fetch(`${API_URL}/api/auth/colaboradores`, {
          method: "POST",
  ...
  // Editar colaborador
  const editarColaborador = useCallback(
    async (colaboradorId, dadosAtualizados) => {
      try {
        setLoading(true);

        console.log(
          "✏️ Editando colaborador:",
          colaboradorId,
          dadosAtualizados
        );

        const response = await fetch(
          `${API_URL}/api/auth/colaboradores/${colaboradorId}`,
          {
            method: "PUT",
            headers: { "Content-Type": "application/json" },
